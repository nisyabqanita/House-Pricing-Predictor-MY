...
#         print(
#             f"T-test between condominium and terrace prices: t-statistic = {t_stat}, p-value = {p_value}"
#         )

#         if p_value < 0.05:
#             print(
#                 "Reject the null hypothesis: There is a significant difference between condominium and terrace prices."
#             )
#         else:
#             print(
#                 "Fail to reject the null hypothesis: No significant difference between condominium and terrace prices."
#             )
...
#             self.data[col] = (
#                 self.data[col].str.strip().str.lower().str.replace(" ", "_")
#             )

#         # Clean 'price' and 'size' columns
#         self.data["price"] = (
#             self.data["price"].str.replace(",", "").str.extract("(\d+)").astype(float)
#         )
#         self.data["size"] = (
#             self.data["size"].str.replace(",", "").str.extract("(\d+)").astype(float)
#         )

#         # Transform 'rooms' column
#         self.data["rooms"] = self.data["rooms"].apply(
#             lambda x: (
#                 sum(map(int, x.split("+")))
#                 if (pd.notnull(x) and all(part.isdigit() for part in x.split("+")))
#                 else np.nan
#             )
#         )
...
#         self.data["car_parks"] = (
#             self.data["car_parks"].fillna(self.data["car_parks"].mean()).astype(int)
#         )
#         self.data["bathrooms"] = (
#             self.data["bathrooms"].fillna(self.data["bathrooms"].mean()).astype(int)
#         )
...
#         }
#         self.data["property_type"] = self.data["property_type"].apply(
#             lambda x: property_mapping.get(x, x)
#         )
...
#         print(
#             "Columns in data before processing:", self.data.columns.tolist()
...
#         cat_feature_names = (
#             self.preprocessor.named_transformers_["cat"]["onehot"]
...
from sklearn.impute import SimpleImputer
from sklearn.metrics import mean_squared_error, r2_score
import joblib  # Add this import for saving the model
...
class HousePricePredictor:
    ...
    def clean_data(self):
        ...
        self.data["price"] = (
            self.data["price"].str.replace(",", "").str.extract(r"(\d+)").astype(float)
        )

        self.data["size"] = (
            self.data["size"].str.replace(",", "").str.extract(r"(\d+)").astype(float)
        )
    ...
    def train_and_evaluate_model(self):
        ...
        joblib_file = "house_price_predictor_model.sav"
